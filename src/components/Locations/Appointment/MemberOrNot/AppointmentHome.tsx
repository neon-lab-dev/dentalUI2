      ...
      <div className="flex flex-col items-center justify-center gap-12 md:py-12 py-6 px-4 md:px-8 xl:px-12 w-[90%] bg-[#EBFAFF] border border-[#333] shadow-sm rounded-2xl md:rounded-[32px] xl:rounded-[48px]">
        ...
        <div className="flex justify-between gap-5 w-full">
          {Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className={`h-[16px] rounded-[30px] flex-1 ${index === 0 ? "bg-[#FF7F50]" : "border border-[#FF7F50]"
                }`}
            ></div>
          ))}
        </div>
        ...
        {[
          ...
          <Button
            ...
            classNames={`w-full lg:w-[670px] h-[73px] md:h-[100px] xl:h-[120px] px-[60px] xl:px-[102px] py-[10px] rounded-[79px] font-Amiri text-base md:text-2xl lg:text-[32px] flex flex-col justify-center items-center gap-[4px] md:gap-0 ${selectedBtn === btn.type ? "text-black" : ""
              }`}
          >
